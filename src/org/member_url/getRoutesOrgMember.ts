...
const OrgMemberListPage = lazy(() =>
  import("#src/org/member_ui/list/OrgMemberListPage.tsx").then((c) => ({
    default: c.OrgMemberListPage,
  })),
)
const OrgMemberEditPage = lazy(() =>
  import("#src/org/member_ui/mutate/OrgMemberEditPage.tsx").then((c) => ({
    default: c.OrgMemberEditPage,
  })),
)
const OrgMemberViewPage = lazy(() => import("#src/ui/pages/TodoPage.tsx").then((c) => ({ default: c.TodoPage })))
const OrgMemberAddPage = lazy(() =>
  import("#src/org/member_ui/mutate/OrgMemberAddPage.tsx").then((c) => ({
    default: c.OrgMemberAddPage,
  })),
)
const OrgMemberRemovePage = lazy(() =>
  import("#src/org/member_ui/mutate/OrgMemberDeletePage.tsx").then((c) => ({
    default: c.OrgMemberDeletePage,
  })),
)
