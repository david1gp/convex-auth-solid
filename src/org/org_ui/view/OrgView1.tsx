...
export function OrgView1(p: OrgViewProps) {
  const org = p.org
  return (
    <section id={orgPageSection.org} class="">
      <div class="flex flex-wrap items-center justify-between gap-4">
        <h1 class="text-2xl font-semibold">{p.org.name}</h1>
        <LinkButton href={urlOrgEdit(org.orgHandle)} variant={buttonVariant.default} icon={formIcon.edit}>
          {ttt("Edit")}
        </LinkButton>
      </div>

      <Show when={org.image}>{(getSrc) => <Img src={getSrc()} alt={`${org.name} logo`} />}</Show>
      <Show when={org.url}>{(getUrl) => <LinkText href={getUrl()}>{getUrl()}</LinkText>}</Show>

      <Show when={org.description}>{(getDescription) => <p class="text-lg mb-4">{getDescription()}</p>}</Show>
    </section>
  )
}
